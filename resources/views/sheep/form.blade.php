                    ...
                    <form action="{{ $action }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-10 space-y-10 border-b border-slate-100 pb-10">
                            ...
                            <div x-data="{ 
                                ...
                                <label class="mb-3 block text-center text-[10px] font-black uppercase tracking-[0.2em] text-[#03235b]">Foto Utama (Cover)</label>
                                <div class="relative group h-48 w-full border-2 border-dashed border-slate-200 rounded-3xl overflow-hidden bg-slate-50 transition-all hover:border-blue-400 hover:bg-blue-50/30 cursor-pointer"
                                    ...
                                    <input type="file" name="photo" x-ref="mainPhotoInput" class="hidden" accept="image/*" @change="handleUpload">
                                    
                                    <div x-show="!preview" class="flex flex-col items-center justify-center h-full space-y-2">
                                        <div class="p-3 bg-white rounded-2xl shadow-sm text-slate-400 group-hover:text-blue-500 group-hover:scale-110 transition-all">
                                            <i data-feather="image" class="w-6 h-6"></i>
                                        </div>
                                        <p class="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Pilih Foto Utama</p>
                                    </div>
                                    ...
                                    <template x-if="preview">
                                        <div class="relative h-full w-full">
                                            ...
                                            <div class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                                                <span class="px-4 py-2 bg-white/30 backdrop-blur-md rounded-full text-white text-[10px] font-black uppercase tracking-widest border border-white/40">Ganti Utama</span>
                                            </div>
                                        </div>
                                    </template>
                                </div>
                            ...
                            <div x-data="{ 
                                ...
                                existingPhotos: [
                                    @foreach($sheep->photos as $p)
                                        { id: {{ $p->id }}, path: '{{ asset('storage/' . $p->path) }}' },
                                    @endforeach
                                ],
                                handleUpload(e) {
                                ...
                                <label class="block text-center text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Galeri Foto Tambahan</label>
                                ...
                                <div class="relative group h-32 w-full border-2 border-dashed border-slate-100 rounded-[2rem] overflow-hidden bg-slate-50/50 transition-all hover:border-emerald-400 hover:bg-emerald-50/30 cursor-pointer flex flex-col items-center justify-center space-y-2"
                                    ...
                                    <input type="file" name="additional_photos[]" x-ref="galleryInput" class="hidden" accept="image/*" multiple @change="handleUpload">
                                    
                                    <div class="p-2 bg-white rounded-xl shadow-sm text-slate-300 group-hover:text-emerald-500 transition-all">
                                        <i data-feather="plus" class="w-5 h-5"></i>
                                    </div>
                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Tambah ke Galeri</p>
                                </div>
                                ...
                                <div class="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 gap-4">
                                    ...
                                    <template x-for="(src, index) in previews" :key="'new-'+index">
                                        <div class="relative aspect-square rounded-2xl overflow-hidden border-2 border-emerald-100 shadow-inner">
                                            <img :src="src" class="h-full w-full object-cover">
                                            <div class="absolute top-1 right-1 w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                                        </div>
                                    </template>
                                </div>
                        ...
                        <div class="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                <label for="age" class="mb-2 block text-sm font-semibold text-slate-700">Umur (Bulan) <span class="text-rose-500">*</span></label>
                                <input type="number" name="age" id="age" value="{{ old('age', $sheep->age) }}" required class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-slate-600 transition-all focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/20" placeholder="Masukkan Umur">
                                @error('age')
                                    <p class="mt-2 text-sm text-rose-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <div x-data="{ 
                                    ...
                                    <div @click="open = !open"
                                        ...
                                        <div class="w-full px-4 py-3 text-[14px] text-slate-700 font-bold" x-text="selected"></div>
                                        <div class="px-3.5 py-3 border-l border-slate-100 bg-slate-50/50 flex items-center justify-center">
                                            <svg class="w-4 h-4 text-slate-400 transition-transform duration-300" :class="open ? 'rotate-180 text-blue-500' : ''" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                                            </svg>
                                        </div>
                                    </div>
                                    <div x-show="open" x-cloak @click.outside="open = false" x-transition class="absolute z-50 w-full mt-1.5 bg-white border border-slate-100 rounded-2xl shadow-2xl overflow-hidden">
                                        <div class="py-1">
                                            <template x-for="option in options" :key="option">
                                                <div @click="select(option)" class="px-4 py-2.5 cursor-pointer hover:bg-slate-50 transition-all duration-200 flex items-center justify-between group">
                                                    <span class="text-[13px] font-bold transition-colors" :class="selected === option ? 'text-blue-600' : 'text-slate-700 group-hover:text-blue-600'" x-text="option"></span>
                            ...
                            <div>
                                <label for="status" class="mb-2 block text-sm font-semibold text-slate-700">Status Domba <span class="text-rose-500">*</span></label>
                                <div x-data="{ 
                                    open: false, 
                                    selected: '{{ old('status', $sheep->status ?? 'tersedia') }}',
                                    options: [
                                        { id: 'tersedia', name: 'Tersedia' },
                                        { id: 'terjual', name: 'Terjual' },
                                        { id: 'sakit', name: 'Sakit' },
                                        { id: 'mati', name: 'Mati' },
                                        { id: 'hilang', name: 'Hilang' }
                                    ],
                                    get selectedName() {
                                        return this.options.find(o => o.id === this.selected)?.name || 'Pilih Status';
                                    },
                                    select(option) {
                                    ...
                                    <div @click="open = !open"
                                        class="relative flex items-center bg-slate-50/50 border border-slate-200 rounded-xl transition-all duration-300 cursor-pointer hover:border-blue-300 focus-within:ring-4 focus-within:ring-blue-500/10"
                                        :class="open ? 'border-blue-400 bg-white ring-4 ring-blue-500/10' : ''">
                                        <div class="w-full px-4 py-3 text-[14px] text-slate-700 font-bold" x-text="selectedName"></div>
                                        <div class="px-3.5 py-3 border-l border-slate-100 bg-slate-50/50 flex items-center justify-center">
                                            <svg class="w-4 h-4 text-slate-400 transition-transform duration-300" :class="open ? 'rotate-180 text-blue-500' : ''" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                                            </svg>
                                    ...
                                    <div x-show="open" x-cloak @click.outside="open = false" x-transition class="absolute z-50 w-full mt-1.5 bg-white border border-slate-100 rounded-2xl shadow-2xl overflow-hidden">
                                        <div class="py-1">
                                            <template x-for="option in options" :key="option.id">
                                                <div @click="select(option)" class="px-4 py-2.5 cursor-pointer hover:bg-slate-50 transition-all duration-200 flex items-center justify-between group">
                                                    <span class="text-[13px] font-bold transition-colors" :class="selected === option.id ? 'text-blue-600' : 'text-slate-700 group-hover:text-blue-600'" x-text="option.name"></span>
                                                    <div x-show="selected === option.id" class="w-4 h-4 bg-blue-100 rounded-full flex items-center justify-center">
                                                        <svg class="w-2.5 h-2.5 text-blue-600" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                                                    </div>
                                                </div>
                                            </template>
                            ...
                            <div x-data="{
                                ...
                                x-init="format()">
                                <label for="price_display" class="mb-2 block text-sm font-semibold text-slate-700">Harga (Rp) <span class="text-rose-500">*</span></label>
                                <input type="text" id="price_display" x-model="display" @input="format" required class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-slate-600 font-bold transition-all focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/20" placeholder="Masukkan Harga Domba">
